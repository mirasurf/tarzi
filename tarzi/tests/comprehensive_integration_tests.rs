...
async fn test_exa_webquery_fallback() {
    ...
    match timeout(Duration::from_secs(60), test_future).await {
        Ok(_) => {
            println!("Exa WebQuery fallback test completed successfully");
        }
        Err(_) => {
...
async fn test_search_limit_edge_cases() {
    ...
    match timeout(Duration::from_secs(60), test_future).await {
        Ok(_) => {
            println!("Search limit edge cases test completed successfully");
        }
        Err(_) => {
            panic!("Test timed out after 60 seconds - this indicates a hanging search operation");
        }
        ...
        let handles: Vec<_> = engines
            ...
            .map(|(i, mut engine)| {
                tokio::spawn(async move {
                    ...
                    // Use ApiQuery mode to avoid WebDriver conflicts in concurrent tests
                    let result = engine.search(&query, SearchMode::ApiQuery, 2).await;
                    (i, result)
                })
            })
            .collect();
...
async fn test_mode_switching() {
    ...
    match timeout(Duration::from_secs(60), test_future).await {
        Ok(_) => {
            println!("Mode switching test completed successfully");
        }
        Err(_) => {
            panic!("Test timed out after 60 seconds - this indicates a hanging search operation");
        }
