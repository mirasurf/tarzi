    ...
    fn parse(&self, _html: &str, limit: usize) -> Result<Vec<SearchResult>> {
        ...
        for i in 0..mock_results_count {
            ...
            let result = SearchResult {
                ...
                snippet: format!(
                    "This is a mock Tavily search result snippet for result {}. In a real implementation, this would be extracted from the HTML using appropriate CSS selectors.",
                    rank
                ),
                rank,
            };
        ...
        info!(
            "Successfully parsed {} Tavily search results",
            results.len()
        );
