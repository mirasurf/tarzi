...
impl SearchResultParser for CustomParser {
    fn parse(&self, html: &str, limit: usize) -> Result<Vec<SearchResult>> {
        info!("Parsing custom search results from HTML for engine: {}", self.engine_name);
        ...
        for i in 0..mock_results_count {
            ...
            let result = SearchResult {
                title: format!("{} Custom Search Result #{} - Mock Title", self.engine_name, rank),
                url: format!("https://example-{}-result-{}.com", self.engine_name.to_lowercase(), rank),
                snippet: format!("This is a mock {} search result snippet for result {}. Custom parser using selectors: container='{}', title='{}', url='{}', snippet='{}'.", 
                    self.engine_name, 
                    rank,
        ...
        info!("Successfully parsed {} custom search results for {}", results.len(), self.engine_name);
