...
impl PySearchEngine {
    ...
    ///     mode (str): Search mode ("webquery", "apiquery")
...
"#;
    ...
    fn test_search_web_function_invalid_mode() {
        setup_python();
        let result = search_web("test", "invalid", 5);
        assert!(result.is_err());
        assert!(
            result
                .unwrap_err()
    ...
    fn test_search_and_fetch_function_invalid_mode() {
        setup_python();
        let result = search_and_fetch("test", "invalid", 5, "plain_request", "html");
        assert!(result.is_err());
        assert!(
            result
                .unwrap_err()
    ...
    fn test_search_and_fetch_function_invalid_fetch_mode() {
        setup_python();
        let result = search_and_fetch("test", "webquery", 5, "invalid", "html");
        assert!(result.is_err());
        assert!(
            result
                .unwrap_err()
    ...
    fn test_search_and_fetch_function_invalid_format() {
        setup_python();
        let result = search_and_fetch("test", "webquery", 5, "plain_request", "invalid");
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Invalid format"));
    }
