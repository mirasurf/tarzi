...
async fn test_search_engine_performance_comparison() {
    ...
    for (engine_name, mode, description) in all_engines {
        ...
        if successful_queries > 0 {
            ...
            println!("  Summary: {}/{} queries successful", successful_queries, test_queries.len());
            println!("  Average response time: {:?}", avg_time);
            println!("  Total results: {}", total_results);
            println!("  Average results per query: {:.1}", total_results as f64 / successful_queries as f64);
        } else {
...
async fn test_search_latency_percentiles() {
    ...
    println!("Performing {} search requests to measure latency distribution...", num_requests);
    ...
    for i in 0..num_requests {
        ...
        match result {
            Ok(results) => {
                ...
                println!("Request {}: {} results in {:?}", i + 1, results.len(), duration);
            }
            Err(e) => {
...
async fn test_search_throughput() {
    ...
    println!("Testing throughput with {} concurrent requests...", concurrent_requests);
    ...
    for handle in handles {
        match handle.await {
            Ok((i, search_result, duration)) => {
                match search_result {
                    Ok(results) => {
                        successful_requests += 1;
                        total_results += results.len();
                        println!("Concurrent request {}: {} results in {:?}", i, results.len(), duration);
                    }
                    Err(e) => {
    ...
    if successful_requests > 0 {
        ...
        println!("  Results per second: {:.2}", total_results as f64 / total_time.as_secs_f64());
    }
}
...
async fn test_search_timeout_behavior() {
    ...
    for timeout_seconds in timeout_values {
        ...
        let start = Instant::now();
        let result = engine.search("timeout test query", SearchMode::WebQuery, 5).await;
        let duration = start.elapsed();
        
        match result {
            Ok(results) => {
                println!("  Completed in {:?} with {} results", duration, results.len());
                assert!(duration.as_secs() <= timeout_seconds as u64 + 5, 
...
async fn test_parser_performance() {
    ...
    let parsers_to_test = vec![
        ...
        ("DuckDuckGo", SearchEngineType::DuckDuckGo, SearchMode::WebQuery),
...
async fn test_error_recovery_performance() {
    ...
    for (query, description) in error_scenarios {
        ...
        match result {
            Ok(results) => {
                println!("  Handled gracefully with {} results in {:?}", results.len(), duration);
            }
            Err(e) => {
        ...
        let recovery_result = engine.search("recovery test", SearchMode::WebQuery, 2).await;
        let recovery_duration = recovery_start.elapsed();
        
        match recovery_result {
            Ok(results) => {
                println!("  Recovery successful: {} results in {:?}", results.len(), recovery_duration);
            }
            Err(e) => {
