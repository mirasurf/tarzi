...
async fn test_search_engine_performance_comparison() {
    ...
    let all_engines = [web_engines, api_engines].concat();
    ...
    for (engine_name, description) in all_engines {
        println!("\n--- Testing {description} ---");
        ...
        match engine_name {
            ...
            "baidu" => {
                if let Ok(key) = env::var("BAIDU_API_KEY") {
                    config.search.baidu_api_key = Some(key);
                }
            }
            _ => {}
        }
...
async fn test_search_timeout_behavior() {
    ...
    for timeout_seconds in timeout_values {
        ...
        let start = Instant::now();
        let result = engine.search("timeout test query", 5).await;
        let duration = start.elapsed();
...
async fn test_error_recovery_performance() {
    ...
    for (query, description) in error_scenarios {
        ...
        let recovery_result = engine.search("recovery test", 2).await;
