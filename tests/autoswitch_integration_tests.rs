...
async fn test_autoswitch_smart_strategy_fallback() {
    ...
    if primary_provider_available && fallback_providers_available {
        ...
        let results = engine.search("test smart fallback", SearchMode::ApiQuery, 3).await;
        ...
        match results {
            Ok(search_results) => {
                ...
                assert!(!search_results.is_empty(), "Smart strategy should return results from available providers");
            }
            Err(e) => {
...
async fn test_autoswitch_none_strategy() {
    ...
    if let Ok(api_key) = env::var("BRAVE_API_KEY") {
        ...
        let mut engine = SearchEngine::from_config(&config);
        let results = engine.search("test no autoswitch", SearchMode::ApiQuery, 3).await;
        
        match results {
            Ok(search_results) => {
                println!("No autoswitch strategy returned {} results", search_results.len());
...
async fn test_autoswitch_with_invalid_primary_provider() {
    ...
    if fallback_available {
        ...
        let mut engine = SearchEngine::from_config(&config);
        let results = engine.search("test fallback from invalid primary", SearchMode::ApiQuery, 2).await;
        
        match results {
            Ok(search_results) => {
                println!("Fallback from invalid primary returned {} results", search_results.len());
                // Should get results from fallback providers
                assert!(!search_results.is_empty(), "Should fallback to valid providers");
            }
            Err(e) => {
...
async fn test_autoswitch_none_with_invalid_provider() {
    ...
    let mut engine = SearchEngine::from_config(&config);
    let results = engine.search("test no fallback with invalid key", SearchMode::ApiQuery, 1).await;
...
async fn test_autoswitch_strategy_parsing() {
    ...
    for (input, expected_behavior) in test_cases {
        ...
        if let Ok(api_key) = env::var("BRAVE_API_KEY") {
            ...
            println!("Created engine with autoswitch strategy: '{}' (expected behavior: '{}')", input, expected_behavior);
...
async fn test_autoswitch_with_all_providers_failing() {
    ...
    let mut engine = SearchEngine::from_config(&config);
    let results = engine.search("test all providers failing", SearchMode::ApiQuery, 1).await;
    ...
    match results {
        Ok(search_results) => {
            ...
            assert!(search_results.is_empty(), "Should return empty results when all real providers fail");
        ...
        Err(error) => {
            ...
            assert!(
                ...
                "Error should indicate provider failures: {}", 
                error
            );
        }
    }
...
async fn test_autoswitch_smart_with_mixed_provider_health() {
    ...
    if valid_provider_available {
        ...
        let mut engine = SearchEngine::from_config(&config);
        let results = engine.search("test mixed provider health", SearchMode::ApiQuery, 2).await;
        
        match results {
            Ok(search_results) => {
                println!("Mixed provider health test returned {} results", search_results.len());
                // Should succeed using valid fallback providers
                assert!(!search_results.is_empty(), "Should use valid fallback providers");
            }
            Err(e) => {
...
async fn test_autoswitch_provider_order() {
    ...
    if multiple_providers >= 2 {
        ...
        let mut engine = SearchEngine::from_config(&config);
        let results = engine.search("test provider order", SearchMode::ApiQuery, 1).await;
        
        match results {
            Ok(search_results) => {
                println!("Provider order test returned {} results", search_results.len());
...
async fn test_autoswitch_performance_comparison() {
    ...
    if let Ok(api_key) = env::var("BRAVE_API_KEY") {
        ...
        let results_smart = engine_smart.search("performance test", SearchMode::ApiQuery, 1).await;
        ...
        let results_none = engine_none.search("performance test", SearchMode::ApiQuery, 1).await;
        ...
        match (results_smart, results_none) {
            ...
            (Err(e1), Err(e2)) => {
                println!("Both strategies failed: {} vs {}", e1, e2);
            }
            _ => {
