...
"""
        try:
            ...
            results = engine.search(test_query, 2)
...
"""
        try:
            ...
            results = engine.search(test_query, 3)
            assert isinstance(results, list)
...
"""
        try:
            ...
            results = engine.search(test_query, 1)
...
"""
        ...
        for limit in limits:
            try:
                results = engine.search(test_query, limit)
                assert isinstance(results, list)
                if limit > 0:
    ...
    def test_api_search_empty_query(self, test_query):
