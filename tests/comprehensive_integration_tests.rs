...
async fn test_exa_webquery_fallback() {
    ...
    match results {
        Ok(search_results) => {
            ...
            println!("Exa WebQuery fallback returned {} results", search_results.len());
...
async fn test_all_engines_parser_factory() {
    ...
    for (engine_type, mode, should_have_real_parser) in test_cases {
        ...
        println!("Testing parser: {} for {:?} in {:?} mode", 
                parser.name(), engine_type, mode);
        ...
        if should_have_real_parser {
            ...
            assert!(parser.supports(&engine_type), 
                   "Parser {} should support {:?}", parser.name(), engine_type);
        }
        ...
        match result {
            Ok(results) => {
                ...
                println!("  Parser returned {} results for empty content", results.len());
            }
            Err(_) => {
...
async fn test_search_engine_error_handling() {
    ...
    match results {
        Ok(search_results) => {
            println!("Invalid engine handled gracefully, returned {} results", search_results.len());
        }
        Err(e) => {
...
async fn test_search_query_edge_cases() {
    ...
    let edge_cases = vec![
        ...
        ("query\nwith\nnewlines", "newlines"),
        ("очень длинный запрос на русском языке", "unicode characters"),
        ("🔍🚀💻", "emoji"),
    ...
    for (query, description) in edge_cases {
        ...
        match results {
            Ok(search_results) => {
                println!("Edge case '{description}' handled gracefully, returned {} results", search_results.len());
                
                // Validate results if any
                for result in search_results {
                    assert!(!result.title.is_empty() || !result.url.is_empty(), 
...
async fn test_search_limit_edge_cases() {
    ...
    for limit in limits {
        ...
        let results = engine.search("test query", SearchMode::WebQuery, limit).await;
        match results {
            Ok(search_results) => {
                if limit == 0 {
                    // Limit 0 should return empty results
                    assert!(search_results.is_empty(), "Limit 0 should return empty results");
                } else {
                    // Results should not exceed limit
                    assert!(search_results.len() <= limit, 
                           "Results ({}) should not exceed limit ({})", 
...
async fn test_search_timeout_handling() {
    ...
    match result {
        Ok(search_result) => {
            match search_result {
                Ok(results) => {
                    println!("Search completed despite short timeout, returned {} results", results.len());
                }
                Err(e) => {
...
async fn test_concurrent_searches() {
    ...
    let handles: Vec<_> = engines.into_iter().enumerate().map(|(i, mut engine)| {
    ...
    for handle in handles {
        match handle.await {
            Ok((i, search_result)) => {
                match search_result {
                    Ok(results) => {
                        println!("Concurrent search {} completed successfully with {} results", i, results.len());
                    }
                    Err(e) => {
...
async fn test_search_result_quality_validation() {
    ...
    let mut engine = SearchEngine::from_config(&config);
    let results = engine
        .search("rust programming language tutorial", SearchMode::WebQuery, 5)
        .await;
    
    match results {
        Ok(search_results) => {
            if !search_results.is_empty() {
                println!("Validating quality of {} search results", search_results.len());
                ...
                for (i, result) in search_results.iter().enumerate() {
                    ...
                    assert!(!result.url.contains("localhost"), "URL should not be localhost");
                    assert!(!result.url.contains("127.0.0.1"), "URL should not be loopback");
                    
                    // Title should be meaningful
                    assert!(result.title.len() > 5, "Title should be reasonably long");
                    assert!(!result.title.to_lowercase().contains("error"), 
                           "Title should not contain error: {}", result.title);
                    ...
                    if !is_relevant {
                        println!("Warning: Result {} may not be relevant to 'rust programming'", i + 1);
                    }
                }
            } else {
...
async fn test_malformed_response_handling() {
    ...
    let malformed_inputs = vec![
        ("<html><body>No search results</body></html>", "HTML without results"),
        ("{}", "Empty JSON"),
    ...
    let parsers = vec![
        ("BingParser", factory.get_parser(&SearchEngineType::Bing, SearchMode::WebQuery)),
        ("DuckDuckGoParser", factory.get_parser(&SearchEngineType::DuckDuckGo, SearchMode::WebQuery)),
        ("DuckDuckGoApiParser", factory.get_parser(&SearchEngineType::DuckDuckGo, SearchMode::ApiQuery)),
    ];
