...
#[tokio::test]
async fn test_exa_web_fallback_external_driver() {
    println!("Testing Exa web fallback with external driver...");

    // Test Exa in web mode (should use fallback parser)
    let mut config = Config::new();
    config.search.engine = "exa".to_string();
    config.fetcher.web_driver = "external".to_string();

    let mut search_engine = SearchEngine::from_config(&config);

    match search_engine.search("test query", 3).await {
        Ok(results) => {
            println!(
                "Exa web fallback with external driver returned {} results",
                results.len()
            );
            // Exa web should use fallback parser and return empty results or minimal results
            assert!(results.len() <= 3);
        }
        Err(e) => {
            println!("Exa web fallback failed as expected: {e}");
            // This is expected for Exa web search
        }
    }

    println!("Exa web fallback test completed successfully");
}

#[tokio::test]
async fn test_exa_web_fallback_self_managed() {
    println!("Testing Exa web fallback with self-managed WebDriver...");

    // Test Exa in web mode with self-managed WebDriver
    let mut config = Config::new();
    config.search.engine = "exa".to_string();
    config.fetcher.web_driver = "self_managed".to_string();
    ...
    match search_engine.search("test query", 3).await {
        Ok(results) => {
            println!(
                "Exa web fallback with self-managed driver returned {} results",
                results.len()
            );
            // Exa web should use fallback parser and return empty results or minimal results
            assert!(results.len() <= 3);
        ...
        Err(e) => {
            println!("Exa web fallback with self-managed driver failed as expected: {e}");
            // This is expected for Exa web search
        }
    }

    println!("Exa web fallback with self-managed driver test completed successfully");
}
...
async fn test_all_engines_parser_factory() {
    let test_future = async {
        ...
        let test_cases = vec![
            ...
            (SearchEngineType::Exa, false), // Fallback parser
        ];

        for (engine_type, should_have_real_parser) in test_cases {
    ...
    // Add 1-minute timeout to prevent hanging
    match timeout(Duration::from_secs(60), test_future).await {
        Ok(_) => {
            println!("Parser factory test completed successfully");
        }
        Err(_) => {
            panic!("Test timed out after 60 seconds - this indicates a hanging operation");
        }
    }
}
...
async fn test_search_engine_error_handling() {
    let test_future = async {
        ...
        // Configure external WebDriver to avoid runtime drop issues
        config.fetcher.web_driver_url = Some("http://localhost:9515".to_string()); // ChromeDriver
        config.fetcher.web_driver = "chrome".to_string();
    ...
    match timeout(Duration::from_secs(60), test_future).await {
        Ok(_) => {
            println!("Search engine error handling test completed successfully");
        }
        Err(_) => {
            panic!("Test timed out after 60 seconds - this indicates a hanging search operation");
        }
    }
}
...
async fn test_search_query_edge_cases() {
    ...
    match timeout(Duration::from_secs(60), test_future).await {
        Ok(_) => {
            println!("Search query edge cases test completed successfully");
        }
        Err(_) => {
            panic!("Test timed out after 60 seconds - this indicates a hanging search operation");
        }
...
#[tokio::test]
async fn test_search_limit_edge_cases_external_driver() {
    let test_future = async {
        let mut config = Config::new();
        config.search.engine = "duckduckgo".to_string();

        // Configure external WebDriver
        config.fetcher.web_driver_url = Some("http://localhost:9515".to_string()); // ChromeDriver
        config.fetcher.web_driver = "chrome".to_string();
    ...
    match timeout(Duration::from_secs(60), test_future).await {
        Ok(_) => {
            println!("Search limit edge cases with external driver test completed successfully");
        }
        Err(_) => {
            panic!("Test timed out after 60 seconds - this indicates a hanging search operation");
        }
...
async fn test_search_limit_edge_cases_self_managed() {
    let test_future = async {
        ...
        // Use self-managed WebDriver (no web_driver_url set)
        config.fetcher.web_driver = "chrome".to_string();
    ...
    match timeout(Duration::from_secs(60), test_future).await {
        Ok(_) => {
            println!(
                "Search limit edge cases with self-managed driver test completed successfully"
            );
        }
        Err(_) => {
            panic!("Test timed out after 60 seconds - this indicates a hanging search operation");
...
async fn test_search_timeout_handling() {
    ...
    let search_future = engine.search("test query", 1);
...
async fn test_concurrent_searches() {
    ...
    let test_future = async {
        ...
        // Use API mode for concurrent testing to avoid WebDriver conflicts
        // Reduce concurrency to prevent resource conflicts
        let engines: Vec<_> = (0..2).map(|_| SearchEngine::from_config(&config)).collect();
        ...
        let handles: Vec<_> = engines
            ...
            .map(|(i, mut engine)| {
                tokio::spawn(async move {
                    ...
                    // Use web mode for concurrent tests
    ...
    match timeout(Duration::from_secs(60), test_future).await {
        Ok(_) => {
            println!("Concurrent searches test completed successfully");
        }
        Err(_) => {
            panic!("Test timed out after 60 seconds - this indicates a hanging search operation");
        }
...
async fn test_search_result_quality_validation() {
    let test_future = async {
        ...
        let mut engine = SearchEngine::from_config(&config);
        let results = engine.search("rust programming language tutorial", 5).await;
    ...
    match timeout(Duration::from_secs(60), test_future).await {
        Ok(_) => {
            println!("Search result quality validation test completed successfully");
        }
        Err(_) => {
            panic!("Test timed out after 60 seconds - this indicates a hanging search operation");
        }
...
async fn test_mode_switching() {
    let test_future = async {
        ...
        println!("Testing web search mode...");
        let web_results = engine.search(query, 3).await;
        match web_results {
            Ok(results) => {
                println!("Web search returned {} results", results.len());
            }
            Err(e) => {
        ...
        println!("Web search test completed successfully");
    };
...
async fn test_malformed_response_handling() {
    ...
    match timeout(Duration::from_secs(60), test_future).await {
        Ok(_) => {
            println!("Malformed response handling test completed successfully");
        }
        Err(_) => {
            panic!("Test timed out after 60 seconds - this indicates a hanging operation");
        }
